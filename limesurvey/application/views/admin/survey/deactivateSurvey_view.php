<?php if (isset($step1))
            ...
            <tr><td><ul>
                ...
                <td>
                    <ul>
						...
						<?php } else { ?>						
							<li><?php $clang->eT("Participants and exisiting reposonse will be imported once the survey is reactivated.");?></li>
							<li><?php $clang->eT("Deleting questions will cause exsisting data to not import, it is recomened you instead hide questions.");?></li>
						<?php }  ?>												
                        ...
                        <li><u><a title='<?php $clang->eT("Export survey results") ?>' href='<?php echo $this->createUrl('admin/export/exportresults/surveyid/'.$surveyid) ?>'></u>
    ...
    <br /><div class='messagebox ui-corner-all'>
		...
		<?php } else { ?>
			<?php $clang->eT("The responses will not be available until the survey is activated again."); ?></p>
		<?php }  ?>
        <p>
		<?php if ($inar_menu_only == false) { ?>
